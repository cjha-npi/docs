...
; (function ($) {
  ...
  const KEY__ROOT_PANE_WIDTH = 'root_pane_width';
  const KEY__LEAF_PANE_WIDTH = 'leaf_pane_width';
  ...
  // Constants
  const MIN_W = 50; // Minimum width (px) of panes
  ...
  const SIDE_NAV_SVG = (() => {
    // svg icon for side-nav in sidebar toggle button
    ...
    frame.setAttribute("shape-rendering", "geometricPrecision");
    svg.appendChild(frame);

    // Translucent column
    var col = document.createElementNS(NS, "rect");
    col.setAttribute("x", "2");
    col.setAttribute("y", "2");
    col.setAttribute("width", "6");
    col.setAttribute("height", "20");
    col.setAttribute("fill", "currentColor");
    col.setAttribute("fill-opacity", "0.3");
    svg.appendChild(col);

    // Line
    const sep = document.createElementNS(NS, "line");
    sep.setAttribute("x1", "9");
    sep.setAttribute("y1", "2");
    sep.setAttribute("x2", "9");
    sep.setAttribute("y2", "22");
    sep.setAttribute("stroke", "currentColor");
    sep.setAttribute("stroke-width", "2");
    sep.setAttribute("vector-effect", "non-scaling-stroke");
  ...
  const DUAL_NAV_SVG = (() => {
    // svg icon for doxy-plus dual nav in sidebar toggle button
    ...
    // Line 1
    const sep1 = document.createElementNS(NS, "line");
    sep1.setAttribute("x1", "9");
    sep1.setAttribute("y1", "2");
    sep1.setAttribute("x2", "9");
    sep1.setAttribute("y2", "22");
    sep1.setAttribute("stroke", "currentColor");
    sep1.setAttribute("stroke-width", "2");
    sep1.setAttribute("vector-effect", "non-scaling-stroke");
    sep1.setAttribute("shape-rendering", "geometricPrecision");
    svg.appendChild(sep1);

    // Line 2
    const sep2 = document.createElementNS(NS, "line");
    sep2.setAttribute("x1", "15");
    sep2.setAttribute("y1", "2");
    sep2.setAttribute("x2", "15");
    sep2.setAttribute("y2", "22");
    sep2.setAttribute("stroke", "currentColor");
    sep2.setAttribute("stroke-width", "2");
    sep2.setAttribute("vector-effect", "non-scaling-stroke");
    sep2.setAttribute("shape-rendering", "geometricPrecision");
  ...
  async function sidebarToggleButton() {
    ...
    function setCssVariable() { // updates doxy-plus.css attribute
      if (_dualNav) document.body.setAttribute('data-dp-dual-nav-active', '');
      else document.body.removeAttribute('data-dp-dual-nav-active');
    ...
    async function setup() {
      ...
      if (!itemSearchBox) {
        console.error(`Sidebar Toggle Button: '#searchBoxPos2' not found`);
        return;
      }
      ...
      if (winWidth >= 768) {
        ...
        if (btn) {
          ...
          btn.replaceChildren(_dualNav ? DUAL_NAV_SVG : SIDE_NAV_SVG);
          //console.log('Sidebar Toggle Button: Reposition');
        }
        else {
          // Create a new toggle button and icon
          btn = document.createElement("button");
          btn.id = 'dp-sidebar-toggle-btn';
          btn.type = "button";
          btn.setAttribute("aria-label", "Sidebar Toggle Button");
          btn.title = 'Toggle Single/Double Navigation Sidebar';
          btn.appendChild(_dualNav ? DUAL_NAV_SVG : SIDE_NAV_SVG);
          btn.addEventListener("click", function () {
            _dualNav = !_dualNav;
            save(KEY__DUAL_NAV, _dualNav);
            btn.replaceChildren(_dualNav ? DUAL_NAV_SVG : SIDE_NAV_SVG);
            setCssVariable();
            //console.log(`Sidebar Toggle Button Clicked: DualNav = ${_dualNav}`);
          });
          itemSearchBox.appendChild(btn);
          //console.log('Sidebar Toggle Button: New Button');
        }
      }
      //else {
      //  console.log('Sidebar Toggle Button: Width < 768');
      //}
  ...
  function checkDualNavLayout(nRoot = _rootPaneWidth, nLeaf = _leafPaneWidth) {
    const wWin = window.innerWidth;
    if (wWin > 767) {
      const maxTotal = wWin - GUTTER_W;
      if (_pageLinks.length > 0) {
        const total = nRoot + nLeaf;
        if (total > maxTotal) {
          const ratio = nRoot / total;
          let nRoot = Math.floor(maxTotal * ratio);
          let nLeaf = maxTotal - nRoot;
          if (nRoot < MIN_W) {
            nRoot = MIN_W;
            nLeaf = maxTotal - nRoot;
          }
          if (nLeaf < MIN_W) {
            nLeaf = MIN_W;
            nRoot = maxTotal - nLeaf;
          }
        }
      ...
      raf = requestAnimationFrame(() => {
        ...
        if (resizerId === 'dp-leaf-pane-resizer') {
          ...
          if (newW < MIN_W) {
            ...
            const nLeaf = MIN_W;
            const nRoot = Math.max(MIN_W, _rootPaneWidth - over);
            if (nLeaf != _leafPaneWidth || nRoot != _rootPaneWidth) {
              checkDualNavLayout(nRoot, nLeaf);
              startX = ev.clientX;
              startW = nLeaf;
            }
          }
            ...
            if (newW > (maxTotal() - _rootPaneWidth))
              newW = maxTotal() - _rootPaneWidth;
            if (newW != _leafPaneWidth) {
              checkDualNavLayout(_rootPaneWidth, newW);
            }
          }
        }
        else {
          ...
          if (_pageLinks.length > 0) {
            ...
            if (newW != _rootPaneWidth) {
              let nLeaf = _leafPaneWidth;
              if (newW < _rootPaneWidth) {
                nLeaf = (_leafPaneWidth + (_rootPaneWidth - newW));
              }
              else {
                if (newW + _leafPaneWidth > maxTotal()) {
                  nLeaf = (maxTotal() - newW);
                }
                else if (_leafPaneWidth > MIN_W) {
                  nLeaf = Math.max(MIN_W, _leafPaneWidth - (newW - _rootPaneWidth));
                }
              }
              checkDualNavLayout(newW, nLeaf);
            }
          }
          else {
            ...
            if (newW != _rootPaneWidth) {
              checkDualNavLayout(newW, _leafPaneWidth);
            }
          }
        }
        raf = null;
  ...
  async function genPageLinks() {
    ...
    if (_pageLinks.length > 0) {
      document.body.setAttribute('data-dp-leaf-nav-active', '');
  ...
  function checkReload() {
    ...
    if (!Array.isArray(_htmlpages) || !_htmlpages.length) return; // No tree data: skip redirect
  ...
  // Keep the “previous URL” upto date
  ...
  async function setupResizeObservers() {
    ...
    if (!cln) document.body.setAttribute('data-dp-doc-header-not-available', '');
    else document.body.removeAttribute('data-dp-doc-header-not-available');

    const pge = document.getElementById('page-nav');
    if (!pge) document.body.setAttribute('data-dp-page-nav-not-available', '');
    else document.body.removeAttribute('data-dp-page-nav-not-available');
  ...
  async function buildTrees() {
    ...
    const [root_pane, leaf_pane] = await Promise.all([waitFor('#dp-root-pane'), waitFor('#dp-leaf-pane')]);
  ...
  Object.defineProperty(consoleObject, 'html-pages_table_format', { // Doxy Plus HTML Pages Table Format
    get() {
  ...
  Object.defineProperty(consoleObject, 'html-pages_group_format', { // Doxy Plus HTML Pages Group Format
    get() {
      console.group('● Doxy-Plus HTML Pages Tree');
  ...
  Object.defineProperty(consoleObject, 'page-links_table_format', { // Doxy Plus Page Links Tree Table Format
    get() {
  ...
  Object.defineProperty(consoleObject, 'page-links_group_format', { // Doxy Plus Page Links Tree Group Format
    get() {
      console.group('● Doxy-Plus Page Links Tree');
  ...
  Object.defineProperty(consoleObject, 'page-link_remarks', { // Doxy Plus Page Links Tree Remarks
    get() {
